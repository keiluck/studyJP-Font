...
2. **详情页** `/articles/[id]`
   - `GET /api/user/articles/{id}`，正文为富文本 HTML 渲染（后端已做 XSS 过滤）。
   - `AudioPlayer` 组件：基于 HTML5 `<audio>` 自定义 UI —— 播放/暂停 IconButton、进度条拖动（MUI Slider + `currentTime`）、当前时间/总时长显示。
   - 一篇文章可能有多条音频，按段落列表展示。

**验收**：筛选+分页正确且刷新后保持；音频可播放/暂停、拖动进度。
