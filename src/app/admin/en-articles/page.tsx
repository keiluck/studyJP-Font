...
import { deleteEnArticle, fetchAdminEnArticles } from "@/api/admin/enArticle";
import { fetchAdminCategories } from "@/api/admin/category";
import type { AdminEnArticleListItem } from "@/types/enArticle";
...
function EnArticleManage() {
  ...
  const [result, setResult] = useState<PageResult<AdminEnArticleListItem> | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [toast, setToast] = useState<string | null>(null);
  const [deleting, setDeleting] = useState<AdminEnArticleListItem | null>(null);
  ...
  const updateQuery = (patch: Record<string, string>) => {
    ...
    router.replace(`/admin/en-articles?${params.toString()}`);
  };
...
export default function EnArticleManagePage() {
  return (
    <Suspense
      fallback={
        <Box sx={{ display: "flex", justifyContent: "center", py: 8 }}>
