...
1. **契约以后端 DTO 为准**：详情接口返回富文本 `content` + 多条 `audios`（`{id,url,title,sortOrder}`），暂无 `sentences` 逐句时间轴。多音频按 `sortOrder` 排序、Chip 切换曲目。
2. **逐段展示**：前端解析富文本为段落（`lib/articleContent.ts`），`SentenceItem` 渲染。录入约定：后台每个日语段落一个 `<p>`，紧跟的中文段落（不含假名）= 该段翻译。
